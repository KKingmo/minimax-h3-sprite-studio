...
async function defaultPythonRunner({ pythonPath, workerPath, workspaceDir, action, payload }) {
...
export function createSpriteWorkspace({
  ...
  async function listJobs() {
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        if (["extracting", "exporting"].includes(job.status)) {
  ...
  async function runPython(action, payload) {
    await requireInstalled();
    return pythonRunner({ pythonPath, workerPath, workspaceDir, action, payload });
  }
  ...
  async function extract(jobId, input) {
    ...
    return runExclusive(async () => {
      ...
      await saveJob(job);
      try {
        const extraction = await runPython("extract", { jobDir, videoPath, ...settings });
  ...
  async function exportJob(jobId, input) {
    ...
    return runExclusive(async () => {
      ...
      await saveJob(job);
      try {
        job.result = await runPython("export", { jobDir, extraction: job.extraction, ...settings });
        job.status = "complete";
        job.extraction = {
          ...job.extraction,
